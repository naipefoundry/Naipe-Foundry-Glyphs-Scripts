...
# --- Window dimensions ---
w = Window((380, 140), "📝 Set License for Custom Fonts")

# --- Instructions ---
w.instruction = TextBox((10, 10, -10, 40), 
...
def okCallback(sender):
    ...
    # Remove trailing " Beta 1", " Beta 2", ..., " Alpha", "alpha", "beta" (case-insensitive)
    family_name = re.sub(r'\s*(beta\s*\d*|alpha)$', '', family_name, flags=re.IGNORECASE)
...
No está permitido enviar o compartir las fuentes {family_name} con ninguna persona u organización, o cualquier tercero que no haya sido contratado directamente por {client_name} como proveedor, socio, franquiciado, contratista o de cualquier otra manera asociado."""
    ...
    Glyphs.showNotification(
        "✅ Font Info Updated",
        f"License (Default, PT, ES) and Trademark set successfully."
    )
    
    w.close()

# --- OK button ---
w.okButton = Button((125, 100, 140, 30), "✅ Apply License", callback=okCallback)
